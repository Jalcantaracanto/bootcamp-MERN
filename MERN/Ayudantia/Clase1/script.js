...
/*: Notación flecha */
misPersonajes.forEach(personajeIterador => {

    /* cada vez que se crea un objeto se agrega a la lista misPersonajesObjeto */
    cartasPersonajes.push(new CartaPersonaje(personajeIterador.name, personajeIterador.rating, personajeIterador.img))

})

/*Notación antigua */
/* misPersonajes.forEach(function (personajeIterador) {
    const personajeAuxObjeto = new CartaPersonaje(personajeIterador.name, personajeIterador.rating, personajeIterador.img)
    cartasPersonajes.push(personajeAuxObjeto)
}) */

/*Queremos agarrar un objeto de la lista cartasPersonajes (random) y ponerla en el tablero */
const repartoUnaCarta = () => {
    
    /* Acceder a un objeto aleatorio del array  */
    const cartaRandom = cartasPersonajes[Math.floor(Math.random() * cartasPersonajes.length)]

    tablero.innerHTML = `<div> <img src=${cartaRandom.img} alt=""> <p>${cartaRandom.nombre}</p> <p>${cartaRandom.rating}</p> </div>`
}

repartir.addEventListener('click', repartoUnaCarta)
